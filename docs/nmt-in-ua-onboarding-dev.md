...
| Викладач (`teacher`) | Призначити тест на `/assign` + результати учнів на `/results` (усі / один учень; теми з гіршим середнім вище; клік по темі → учні від гіршого бала) + «Мої учні» на `/students` + консультації + візитка. Без пункту «Тест за обраною темою» в меню. |
...
- Вийди, зайди як `demo-teacher`, на `/students` додай `demo-student`, на `/assign` признач тему «на зараз», на `/results` глянь «усі учні» і клік по темі. На `/account` заповни візитку, постав «опублікувати», відкрий `/t/{slug}` інкогніто.
