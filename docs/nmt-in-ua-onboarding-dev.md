...
| Викладач (`teacher`) | Призначити тест на `/assign` (тема + учні + термін) + «Мої учні» на `/students` + черга консультацій на `/consultations` + публічна візитка на `/account` (`/t/{slug}`). Без пункту «Тест за обраною темою» в меню. |
...
| `demo-teacher` | `demo123` | Викладач | Призначення на `/assign`; «Мої учні» на `/students`; візитка на `/account` |
...
| `/assign` | Лише teacher/admin | Призначити тест за темою: зараз / дата-час, галочки учнів, список призначень і статуси |
...
- Вийди, зайди як `demo-teacher`, на `/students` додай `demo-student`, на `/assign` признач тему «на зараз». На `/account` заповни візитку, постав «опублікувати», відкрий `/t/{slug}` інкогніто.
